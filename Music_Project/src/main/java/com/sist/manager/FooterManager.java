...
public class FooterManager {
	...
	public List<FindVO> footerData(String fd) {
        String clientId = naverId; //애플리케이션 클라이언트 아이디
        String clientSecret = naverapiKey; //애플리케이션 클라이언트 시크릿
        ...
        try {
			...
			for(int i=0;i<arr.size();i++) {
				...
				vo.setTitle(String.valueOf(obj.get("title")));
				vo.setLink(String.valueOf(obj.get("link")));
				list.add(vo);
			}
		} catch (Exception e) {
			e.printStackTrace();
